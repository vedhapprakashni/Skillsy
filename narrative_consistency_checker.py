...
Or with environment variable for API key:
  ...
  python narrative_consistency_checker.py \\
    ...
    print(f"   Novels Directory: {args.novels_dir}")
    print(f"   Novel Mapping: {args.novel_mapping}")
    print(f"   Output File: {args.output}")
    print(f"   API Key: {'*' * min(20, len(api_key))} (hidden)")
    ...
    if not os.path.isfile(args.test_csv):
        print(f"❌ Error: Test CSV path is not a file: {args.test_csv}")
        sys.exit(1)
    
    # Check novels directory
    if not os.path.exists(args.novels_dir):
        print(f"❌ Error: Novels directory not found: {args.novels_dir}")
        sys.exit(1)
    if not os.path.isdir(args.novels_dir):
        print(f"❌ Error: Novels path is not a directory: {args.novels_dir}")
        sys.exit(1)
    
    # Check novel mapping file
    if not os.path.exists(args.novel_mapping):
        print(f"❌ Error: Novel mapping file not found: {args.novel_mapping}")
        sys.exit(1)
    if not os.path.isfile(args.novel_mapping):
        print(f"❌ Error: Novel mapping path is not a file: {args.novel_mapping}")
        sys.exit(1)
    
    print("✅ All input files/directories validated")
    ...
    try:
        ...
        required_columns = ['id', 'book_name', 'char']
        missing_columns = [col for col in required_columns if col not in test_df.columns]
        if missing_columns:
            print(f"❌ Error: Test CSV missing required columns: {missing_columns}")
            print(f"   Available columns: {list(test_df.columns)}")
            sys.exit(1)
        
        # Check for content/caption column
        if 'content' not in test_df.columns and 'caption' not in test_df.columns:
            print("⚠️  Warning: Neither 'content' nor 'caption' column found. Backstories may be empty.")
        
    except pd.errors.EmptyDataError:
        print(f"❌ Error: Test CSV file is empty: {args.test_csv}")
        sys.exit(1)
    except Exception as e:
    ...
    try:
        ...
        print(f"   Books: {', '.join(novel_mapping.keys())}")
        
        # Validate mapped files exist
        missing_files = []
        for book_name, filename in novel_mapping.items():
            file_path = os.path.join(args.novels_dir, filename)
            if not os.path.exists(file_path):
                missing_files.append((book_name, filename))
        
        if missing_files:
            print("\n⚠️  Warning: Some mapped novel files not found:")
            for book_name, filename in missing_files:
                print(f"   - {book_name}: {filename}")
            print("   These will be skipped during indexing")
